    ...
    const handleSubmit = () => {
        useEffect(() => {
            async () => {
                try {
                    setLoading(true);
                    const response = await axios.post(`http://localhost:3000/support/addperson`, {
                        data
                    }, {
    ...
    return (
            ...
            <div className=" h-fit mx-auto border-2 rounded-xl md:p-6 md:w-[85%] w-[95%] ">
                <div className="flex flex-col space-y-9 p-3 mx-auto">
                    <div className="flex items-center sm:space-x-4 text-white">
                        ...
                        <select name="Role" value={role} onChange={(e) => setRole(e.target.value)} className=" mx-2 text-black" id="Role">
                            <option value="">Responsibility</option>
                            <option value="volunteer">Volunteer</option>
                            <option value="coordinator">Coordinator</option>
                            <option value="incharge">Incharge</option>
                            <option value="organiser">Organiser</option>
                            <option value="organiser">Timekeeper</option>
                            <option value="organiser">Recorder</option>
                        </select>
                    </div>
                    <div className="flex flex-col sm:space-x-4 text-white ">
                        <label htmlFor="Event-Details" className="text-lg  text-center font-bold">
                            Event Details
                        </label>
        ...
        * for faculty
                    ...
                    <div className="checkbox flex space-x-3 ">
                        <input type="checkbox" name="checked" id="checked" className="w-5" required />
                    ...
                    <div className="flex items-center justify-center pb-4 hover:scale-95 transition-all delay-100">
                        <button onClick={handleSubmit} className="px-8 py-3 bg-slate-700 text-white rounded-lg font-semibold shadow-md shadow-blue-400">Submit</button>
                    </div>
                </div>
            </div>
        </div>
